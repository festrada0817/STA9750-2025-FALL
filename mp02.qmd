...
prop_health<-ggplot(regional_health_share)+
  ...
  labs(
    ...
    subtitle = "Each bar shows what proportion of all U.S. health care and social assistance workers are located in \n each region, for every year.",
     caption = "Source: U.S. Bureau of Labor Statistics, Quarterly Census of Employment and Wages (QCEW), 2009–2023. \n Data collected and downloaded at https://www.bls.gov/cew/.",
    x = "Year",
...
# Join population and households and calculate average household size
average_household_size <- POPULATION %>%
  select(GEOID, NAME, year, population) %>%
  left_join(HOUSEHOLDS %>% select(GEOID, year, households),
            by = c("GEOID", "year")) %>%
  mutate(
    avg_household_size = population / households,
    highlight_cities = NAME %in% c("New York-Newark-Jersey City, NY-NJ-PA Metro Area", 
                                   "Los Angeles-Long Beach-Anaheim, CA Metro Area")
  ) %>%
  group_by(GEOID)   # add this grouping step

#plot
ggplot(average_household_size, aes(x = year, y = avg_household_size, group = GEOID, color = highlight_cities)) +
  ...
  labs(
    ...
    caption = "Source: U.S. Census Bureau, American Community Survey 5-Year Estimates, Tables B01003 (Population) and B11001 (Household Type), accessed October 2025 at https://data.census.gov/"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold", size = 16),
...
inst_summary %>%
  rename(
    ...
    "Permits per 1,000 Residents" = growth_instant,
    "Instantaneous Growth" = inst_scaled
  ) %>%
  mutate(
    `Permits per 1,000 Residents` = round(`Permits per 1,000 Residents`, 2),
    `Instantaneous Growth` = round(`Instantaneous Growth`, 2)
  ) %>%
  datatable(
  ...
  formatCurrency("Housing Units Permitted", currency = "", interval = 3, mark = ",", digits = 0)
...
rate_summary %>%
  rename(
    CBSA = GEOID,
    "Population Growth (5-Year)" = pop_growth_5yr,
    "Housing Units Permitted" = new_housing_units_permitted,
  ...
  mutate(
    `Permits per Resident Added` = round(`Permits per Resident Added`, 2),
    `Rate-Based Growth (Scaled 0–100)` = round(`Rate-Based Growth (Scaled 0–100)`, 2)
  ) %>%
  datatable(
    options = list(searching = FALSE, info = FALSE),
    caption = "Table 4: CBSAs with Highest Housing Growth Relative to Population Gain"
  ) %>%
  formatCurrency("Population Growth (5-Year)", currency = "", interval = 3, mark = ",", digits = 0) %>%
  formatCurrency("Housing Units Permitted", currency = "", interval = 3, mark = ",", digits = 0)
...
comp_summary %>%
  rename(
    CBSA = GEOID,
    "Composite Housing Growth Score" = composite_score,
    "CBSA Name" = NAME
  ) %>%
  mutate(
    `Composite Housing Growth Score` = round(`Composite Housing Growth Score`, 2)
  ) %>%
  datatable(
    options = list(searching = FALSE, info = FALSE),
